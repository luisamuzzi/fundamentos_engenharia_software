...
from src.fundamentos_engenharia_software.config import (
    PROCESSED_DATA_PATH, X_TRAIN_IMPUTED_DATA_PATH, X_TEST_IMPUTED_DATA_PATH, Y_TRAIN_DATA_PATH, Y_TEST_DATA_PATH, COLS_TO_USE
    )
...
def scale_missing_columns(X_train, X_test, missing_columns):
    ...
    X_train_scaled[missing_columns] = scaler.fit_transform(X_train[missing_columns])
    X_test_scaled[missing_columns] = scaler.transform(X_test[missing_columns])

    return X_train_scaled, X_test_scaled

def impute_missing_data(X_train, X_test, X_train_scaled, X_test_scaled, missing_columns):
...
def impute_and_scale():
    ...
    X_train_scaled, X_test_scaled = scale_missing_columns(X_train, X_test, missing_columns)

    X_train_imputed, X_test_imputed = impute_missing_data(X_train, X_test, X_train_scaled, X_test_scaled, missing_columns)
