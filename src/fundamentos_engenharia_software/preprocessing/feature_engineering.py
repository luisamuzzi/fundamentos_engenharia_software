...
from src.fundamentos_engenharia_software.config import RAW_DATA_PATH, PROCESSED_DATA_PATH
...
def create_other_category_values(df, lista_categorias_outros):
    ...
    df_copy.loc[df_copy["grupo_categorias"].isin(lista_categorias_outros), "grupo_categorias"] = "categorias_outros"
...
def create_features_and_encode():
    ...
    df_with_other_category = create_other_category_values(df, other_categories_list)
    
    df_with_countries_grouped = group_countries(df_with_other_category, countries_to_keep=['BR', 'AR'])
